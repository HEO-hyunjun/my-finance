...
import type { StagedEntry } from '@/entities/import/model/types';
              ...
              return (
                <tr key={row.id} className="border-t align-middle">
                  ...
                  <td className="whitespace-nowrap px-3 py-2 text-xs text-muted-foreground">
                    {new Date(row.transacted_at).toLocaleDateString('ko-KR', { month: '2-digit', day: '2-digit' })}
                  </td>
                  <td className="max-w-[200px] px-3 py-2">
                    <span className="block truncate">{row.description ?? '—'}</span>
                  </td>
                  <td className={`whitespace-nowrap px-3 py-2 text-right font-medium tabular-nums ${amount < 0 ? 'text-destructive' : 'text-green-600'}`}>
                    {formatKRW(amount)}
                  </td>
                  <td className="px-3 py-2">
                    <Select
      ...
      {editable ? (
        <div className="mt-4 flex items-center justify-between">
          ...
          <Button
            ...
            onClick={() => commit.mutate(createAdjustment && hasBalanceDiff, { onSuccess: onBack })}
          >
