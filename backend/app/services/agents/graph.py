...
2. **뉴스 분석 에이전트** (ask_news_agent): 금융 뉴스, 시장 동향, 경제 이슈, 종목 뉴스
3. **가계부 분석 에이전트** (ask_budget_agent): 예산, 지출, 수입, 고정비, 할부금, 가계 분석

## 위임 전략
- 질문 내용에 맞는 에이전트에 위임합니다.
- 복합 질문은 여러 에이전트에 동시 위임합니다 (예: "내 자산 현황과 관련 뉴스" → 자산 + 뉴스)
...
ORCHESTRATOR_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "ask_news_agent",
            "description": "뉴스/시장 동향 관련 질문을 뉴스 분석 에이전트에게 위임합니다. 최신 뉴스, 시장 트렌드, 경제 이슈, 종목 뉴스 관련 질문에 사용합니다.",
            "parameters": {
                "type": "object",
                "properties": {
                    "question": {
                        "type": "string",
                        "description": "뉴스 에이전트에게 전달할 구체적인 질문",
                    },
                },
                "required": ["question"],
            },
