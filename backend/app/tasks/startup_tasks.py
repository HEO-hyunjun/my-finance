...
def run_missed_tasks():
    ...
    # 1. DB 기반 통합 보상 (정기수입 / 자동이체 / 고정비 / 할부)
    try:
        celery_app.send_task(
            "app.tasks.compensation_tasks.compensate_missed_tasks",
            countdown=10,
        )
        logger.info("Compensation task queued: compensate_missed_tasks (in 10s)")
    except Exception as e:
        logger.warning(f"Failed to queue compensation task: {e}")

    # 2. 기타 일일 태스크 (오늘 기준 실행, 중복 시 자체 스킵)
    OTHER_TASKS = [
        ("app.tasks.interest_tasks.record_daily_parking_interest", 15),
        ("app.tasks.market_tasks.warm_market_cache", 20),
    ]
    for task_name, countdown in OTHER_TASKS:
        try:
